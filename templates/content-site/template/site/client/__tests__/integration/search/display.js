...
it( 'display', async() => {
    ...
    history.pushState( {}, '', 'http://client.example.com/search/1?q=content' );
...
it( 'ssr', async() => {
    ...
    ;
    setupSSR();
    await app.ssrAndCheck( 'http://client.example.com/search/1?q=content' );
    expect( axios.mocks.get ).toHaveBeenCalledTimes( 1 );
    expect( axios.mocks.get.mock.calls[ 0 ] ).toEqual( [
        '/articles',
        { limit: 9, offset: 0, q: 'content' }
...
it( 'ssr & rehydrate', async() => {
    ...
    await app.ssrAndRehydrate( 'http://client.example.com/search/1?q=content' );
