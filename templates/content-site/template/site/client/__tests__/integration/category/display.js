...
it( 'display', async() => {
    ...
    history.pushState( {}, '', 'http://client.example.com/category/pervya/1' );
...
it( 'ssr', async() => {
    ...
    ;
    setupSSR();
    await app.ssrAndCheck( 'http://client.example.com/category/pervya/1' );
    expect( axios.mocks.get ).toHaveBeenCalledTimes( 1 );
    expect( axios.mocks.get.mock.calls[ 0 ] ).toEqual( [
        '/articles',
        { 'limit': 9, 'offset': 0, 'category': 'pervya' }
...
it( 'ssr & rehydrate', async() => {
    ...
    await app.ssrAndRehydrate( 'http://client.example.com/category/pervya/1' );
