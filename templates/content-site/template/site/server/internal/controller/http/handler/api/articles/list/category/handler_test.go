	...
	testCases := []struct {
	...
	}{
		{
			...
			service: func(t mockConstructorTestingTNewMockArticlesService) *MockArticlesService {
				...
				m.On("ArticleListForCategory", mock.Anything, "test-category", int64(10), int64(30)).Return(&model.PaginatedArticleForCategory{
					Total: 1,
			...
			request:      httptest.NewRequest(http.MethodGet, "/articles?offset=10&limit=30&category=test-category", nil),
		...
		},
		{
			...
			request:      httptest.NewRequest(http.MethodGet, "/articles?limit=-1&category=test", nil),
		...
		},
		{
			name:         "empty category",
			service:      NewMockArticlesService,
			request:      httptest.NewRequest(http.MethodGet, "/articles?category=", nil),
			expectedCode: http.StatusBadRequest,
			expectedBody: `{"Status":"Bad Request","Messages":["empty category"]}`,
		},
		{
			name: "internal server error",
			service: func(t mockConstructorTestingTNewMockArticlesService) *MockArticlesService {
				m := NewMockArticlesService(t)
				m.On("ArticleListForCategory", mock.Anything, "test-category", int64(10), int64(30)).Return(nil, errors.New("fail get articles"))
				return m
			},
			request:      httptest.NewRequest(http.MethodGet, "/articles?offset=10&limit=30&category=test-category", nil),
