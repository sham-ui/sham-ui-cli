	...
	testCases := []struct {
	...
	}{
		{
			...
			service: func(t mockConstructorTestingTNewMockArticlesService) *MockArticlesService {
				...
				m.On("ArticleListForTag", mock.Anything, "test-tag", int64(10), int64(30)).Return(&model.PaginatedArticleForTag{
					Total: 1,
			...
			request:      httptest.NewRequest(http.MethodGet, "/articles?offset=10&limit=30&tag=test-tag", nil),
		...
		},
		{
			...
			request:      httptest.NewRequest(http.MethodGet, "/articles?limit=-1&tag=test", nil),
		...
		},
		{
			name:         "empty tag",
			service:      NewMockArticlesService,
			request:      httptest.NewRequest(http.MethodGet, "/articles?tag=", nil),
			expectedCode: http.StatusBadRequest,
			expectedBody: `{"Status":"Bad Request","Messages":["empty tag"]}`,
		},
		{
			name: "internal server error",
			service: func(t mockConstructorTestingTNewMockArticlesService) *MockArticlesService {
				m := NewMockArticlesService(t)
				m.On("ArticleListForTag", mock.Anything, "test-tag", int64(10), int64(30)).Return(nil, errors.New("fail get articles"))
				return m
			},
			request:      httptest.NewRequest(http.MethodGet, "/articles?offset=10&limit=30&tag=test-tag", nil),
