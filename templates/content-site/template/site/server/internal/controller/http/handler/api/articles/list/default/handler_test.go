	...
	testCases := []struct {
	...
	}{
		{
			...
			service: func(t mockConstructorTestingTNewMockArticlesService) *MockArticlesService {
				...
				m.On("Articles", mock.Anything, int64(10), int64(30)).Return(&model.PaginatedArticles{
					Total: 1,
			...
			request:      httptest.NewRequest(http.MethodGet, "/articles?offset=10&limit=30", nil),
		...
		},
		{
			...
			request:      httptest.NewRequest(http.MethodGet, "/articles?limit=-1", nil),
		...
		},
		{
			...
			service: func(t mockConstructorTestingTNewMockArticlesService) *MockArticlesService {
				m := NewMockArticlesService(t)
				m.On("Articles", mock.Anything, int64(10), int64(30)).Return(nil, errors.New("fail get articles"))
				return m
			},
			request:      httptest.NewRequest(http.MethodGet, "/articles?offset=10&limit=30", nil),
