...
func TestHandler_ServeHTTP(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:    "success",
			request: httptest.NewRequest(http.MethodPost, "/", strings.NewReader(goodBody)),
			slugifyService: func(t mockConstructorTestingTNewMockSlugifyService) *MockSlugifyService {
			...
			articleService: func(t mockConstructorTestingTNewMockArticleService) *MockArticleService {
				...
				m.
					On("Create", mock.Anything, article, tags).
					Return(model.ArticleID("1"), nil).
					Once()
				return m
			},
			expectedCode:           http.StatusCreated,
			expectedBody:           `{"Status":"Article created"}`,
			expectedLoggerMessages: []testlogger.Message{},
		},
		{
			name:                   "invalid json",
		...
		},
		{
			...
			request: httptest.NewRequest(http.MethodPost, "/", strings.NewReader(goodBodyWithoutTags)),
			slugifyService: func(t mockConstructorTestingTNewMockSlugifyService) *MockSlugifyService {
				m := NewMockSlugifyService(t)
				m.
					On("SlugifyArticle", mock.Anything, "new article").
			...
			articleService: func(t mockConstructorTestingTNewMockArticleService) *MockArticleService {
				m := NewMockArticleService(t)
				m.
					On("Create", mock.Anything, article, []model.Tag{}).
					Return(model.ArticleID(""), model.ErrArticleTitleAlreadyExists).
					Once()
				return m
			},
			expectedCode: http.StatusBadRequest,
		...
		},
		{
			...
			articleService: func(t mockConstructorTestingTNewMockArticleService) *MockArticleService {
				...
				m.
					On("Create", mock.Anything, article, []model.Tag{}).
					Return(model.ArticleID(""), model.ErrArticleSlugAlreadyExists).
					Once()
				return m
			},
			expectedCode: http.StatusBadRequest,
		...
		},
		{
			...
			articleService: func(t mockConstructorTestingTNewMockArticleService) *MockArticleService {
				...
				m.
					On("Create", mock.Anything, article, []model.Tag{}).
					Return(model.ArticleID(""), model.ErrCategoryNotFound).
					Once()
				return m
			},
			expectedCode: http.StatusBadRequest,
