...
func TestHandler_ArticleList(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			articleService: func(t mockConstructorTestingTNewMockArticleService) *MockArticleService {
				...
				m.
					On("TotalForQuery", mock.Anything, query).
					Return(3, nil).
					Once()
				m.
					On("FindShortInfoWithCategoryForQuery", mock.Anything, query, int64(1), int64(2)).
					Return([]model.ArticleShortInfoWithCategory{firstArticle, secondArticle}, nil).
					Once()
				return m
			},
			req: &proto.ArticleListForQueryRequest{Offset: 1, Limit: 2, Query: query},
			expectedResponse: &proto.ArticleListResponse{
	...
	for _, test := range testCases {
		t.Run(test.name, func(t *testing.T) {
			...
			// Action
			resp, err := h.ArticleListForQuery(context.Background(), test.req)
